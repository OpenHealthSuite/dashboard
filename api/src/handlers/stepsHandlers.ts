...
const getUserSteps = async (userId: string, req: Request, res: Response<any, DashboardLocals>) => {
  ...
  if (!res.locals.dataProvider.dailyStepsProvider) {
    return res.sendStatus(404)
  }

  const steps = await res.locals.dataProvider.dailyStepsProvider(userId, new Date(date))
...
const getUserStepsRange = async (userId: string, req: Request, res: Response<any, DashboardLocals>) => {
  ...
  if (!res.locals.dataProvider.dateRangeStepProvider) {
    return res.sendStatus(404)
  }
  const steps = await res.locals.dataProvider.dateRangeStepProvider(userId, new Date(dateStart), new Date(dateEnd))
