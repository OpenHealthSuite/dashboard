...
describe('FitbitStatProvider', () => {
  describe('dateRangeSleepProvider', () => {
    ...
    test('Two Entries in a Day :: Merges Values', async () => {
      // Arrange
      const fakeData = {
        sleep: [
          {
          ...
          {
            dateOfSleep: '2021-12-12T09:23:38.487Z',
            minutesAwake: 5,
            minutesAsleep: 7,
            levels: {
              summary: {
      ...
      expect(dateOne.sleep.awake).toBe(1 + 5)
      expect(dateOne.sleep.asleep).toBe(3 + 7)
      expect(dateOne.sleep.rem).toBe(5 + 9)
