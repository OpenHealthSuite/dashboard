...
export class UserServiceTokenRepository<Token extends { expires_in: number }> {
  ...
  async createUserToken (userId: string, token: Token): Promise<Result<Token, string>> {
    try {
  ...
  async getUserToken (userId: string): Promise<Result<{ raw_token: Token, last_updated: Date } | null, string>> {
  ...
  async updateUserToken (userId: string, token: Token): Promise<Result<Token, string>> {
    try {
      const insertQuery = `INSERT INTO paceme.${this._tableName} (service_id, paceme_user_id, raw_token, last_updated, expires_in) VALUES (?, ?, ?, ?, ?);`
      await this._cassandraClient.execute(insertQuery, [this._serviceId, userId, JSON.stringify(token), new Date(), token.expires_in], { prepare: true })
      return ok(token)
  ...
  async getTokensThatExpireBefore (date: Date): Promise<Result<{ raw_token: Token, paceme_user_id: string }[], string>> {
